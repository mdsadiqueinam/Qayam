...
fun OffsetsSection(
    ...
    SettingsSectionCard(title = "Prayer Time Adjustments", icon = Icons.Default.Tune, modifier = modifier) {
...
@Composable
private fun OffsetRow(prayer: PrayerType, offset: Int, onUpdateOffset: (PrayerType, Int) -> Unit) {
    Row(
        modifier = Modifier
            .fillMaxWidth()
        ...
        Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            OffsetStepButton(
                ...
                description = "Decrease ${prayer.displayName} offset by one minute",
            ...
            OffsetStepButton(
                ...
                description = "Increase ${prayer.displayName} offset by one minute",
                onClick = { onUpdateOffset(prayer, offset + 1) },
            )
        }
    }
...
@Composable
private fun OffsetStepButton(image: ImageVector, description: String, onClick: () -> Unit) {
    Surface(shape = CircleShape, color = MaterialTheme.colorScheme.surfaceVariant, modifier = Modifier.size(32.dp)) {
        IconButton(onClick = onClick) {
            Icon(image, contentDescription = description, modifier = Modifier.size(16.dp))
        }
    }
}
